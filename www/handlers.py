...
def text2html(text):
    lines=map(lambda s:s.replace('&','&amp').replace('<','&lt').replace('>','&gt'),filter(lambda s:s.strip!='',text.split('\n')))
    return ''.join(lines)

def user2cookie(user,max_age):
    '''
    通过user和max_age生成cookie字符串
    '''
...
async def authenticate(*,email,passwd):
    ...
    #检查passwd
    sha1=hashlib.sha1()
    sha1.update(user.id.encode('utf-8'))#update有什么用?
    sha1.update(b':')
